...
#include "rclcpp/msg/dummy_message.hpp"
...
using DummyMessage = rclcpp::msg::DummyMessage;
...
/**
 * DummtMessage publisher node: used to publish DummyMessages with `header` value set
 */
...
public:
  ...
  : Node(name)
  {
    publisher_ = create_publisher<DummyMessage>(topic, 10);
    publish_timer_ = this->create_wall_timer(
      publish_period, [this]() {
        this->publish_message();
      });
...
private:
  ...
  rclcpp::Publisher<DummyMessage>::SharedPtr publisher_;
  std::atomic<size_t> number_published_{0};
  rclcpp::TimerBase::SharedPtr publish_timer_;
};
...
/**
 * DummyMessage subscriber node: used to create subscriber topic statistics requirements
 */
...
public:
  ...
  : Node(name)
  {
    ...
    auto callback = [this](DummyMessage::UniquePtr msg) {
        this->receive_message(*msg);
      };
    subscription_ = create_subscription<DummyMessage,
        std::function<void(DummyMessage::UniquePtr)>>(
      topic,
      rclcpp::QoS(rclcpp::KeepAll()),
      callback,
      options);
...
private:
  void receive_message(const DummyMessage &) const
  {
  }
  rclcpp::Subscription<DummyMessage>::SharedPtr subscription_;
};
...
{
  // create a DummyMessage publisher
