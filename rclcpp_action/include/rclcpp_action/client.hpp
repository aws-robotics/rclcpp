...
public:
   ...
   * \param[in] client_options Pptions to pass to the underlying `rcl_action_client_t`.
   */
  Client(
    rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node_base,
    rclcpp::node_interfaces::NodeGraphInterface::SharedPtr node_graph,
  ...
  /**
   * If the goal is accepted by an action server, the result of the returned future is set to a
   * `ClientGoalHandle`.
   * If the goal is rejected by an action server, then the future result is set to a `nullptr`.
   *
  ...
  /// Asynchronsouly request for a goal to be canceled.
  /**
